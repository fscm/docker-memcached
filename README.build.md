...
Docker image with Memcached.

## Synopsis

This script will create a Docker image with Memcached installed.

The Docker image resulting from this script should be the one used to
instantiate a Memcached server.
...
- `<IMAGE>` - *[required]* The container name (e.g.: "memcached").
...
```shell
docker image build --force-rm --no-cache --progress plain --tag johndoe/my_memcached:latest .
```
